...
void GPIO_ExternalSWInit(uint8 portID, uint8 pinID)
{
    switch(portID)
    {
        case 0:
            ...
            GPIO_PORTA_AMSEL_REG &= ~(1<<pinID);
            GPIO_PORTA_PCTL_REG &= ~(0x0000000F << (pinID * 4));
            GPIO_PORTA_DIR_REG &= ~(1<<pinID);
            GPIO_PORTA_AFSEL_REG &= ~(1<<pinID);
            GPIO_PORTA_PUR_REG |= (1<<pinID);
            GPIO_PORTA_DEN_REG |= (1<<pinID);
            break;

        case 1:
            /* Same Implementation as "case 0" but with different PORT */
        break;
...
void GPIO_ExternalLEDInit(uint8 portID, uint8 pinID)
{
    switch(portID)
    {
        ...
        case 1:   /* PORTB*/
            ...
            GPIO_PORTB_AMSEL_REG &= ~(1<<pinID);
            GPIO_PORTB_PCTL_REG &= ~(0x0000000F << (pinID * 4));
            GPIO_PORTB_DIR_REG |= (1<<pinID);
            GPIO_PORTB_AFSEL_REG &= ~(1<<pinID);
            GPIO_PORTB_DEN_REG |= (1<<pinID);
            break;
...
void GPIO_ExternalSWEdgeTriggeredInterruptInit(uint8 pinID, uint8 portID)
{
    ...
    {
        case 0:
            GPIO_PORTA_IS_REG &= ~(1<<pinID);
            GPIO_PORTA_IBE_REG &= ~(1<<pinID);
            GPIO_PORTA_IEV_REG &= ~(1<<pinID);
            GPIO_PORTA_ICR_REG |= (1<<pinID);
            GPIO_PORTA_IM_REG |= (1<<pinID);
            ...
            NVIC_EN0_REG |= (1<<0); /* Enable NVIC Interrupt for GPIO PORTA by set bit number 0 in EN0 Register */
            break;

        case 1:
            /* Same Implementation as "case 0" but with different PORT and some modifications in the last 2 lines */
        break;
